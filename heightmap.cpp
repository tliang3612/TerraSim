...
Heightmap::Heightmap(float size, int resolution, DataFactory dataFactory) : m_heightmapSize(size), m_heightmapResolution(resolution) {
...
void Heightmap::GenerateHeightsUsingNoise(int noiseType, bool setSeed) {
    if (setSeed) {
        m_noise.SetSeed(static_cast<int>(time(nullptr)));
    }
